...
public class UnitFactoryModif extends UnitBlock {
    public int[] capacities = {};
    public float line1Speed = 2;
    public float line2Speed = 2;
    public Color color;

    public Seq<UnitPlan> plans = new Seq<>(4);

    public UnitFactoryModif(String name){
        super(name);
        update = true;
        hasPower = true;
        hasItems = true;
    ...
    public void setBars(){
        super.setBars();
        addBar("Manufacturing Progress", (UnitFactoryBuild e) -> new Bar("Manufacturing Progress", Color.valueOf("#8875ff"), e::fraction));
    ...
    public class UnitFactoryBuild extends UnitBuild{
        ...
        public void buildConfiguration(Table table){
            ...
            if(units.any()){
                ItemSelection.buildTable(UnitFactoryModif.this, table, units, () -> currentPlan == -1 ? null : plans.get(currentPlan).unit, unit -> configure(plans.indexOf(u -> u.unit == unit)), selectionRows, selectionColumns);
        ...
        public void draw(){
            ...
            if(currentPlan != -1){
                UnitFactoryModif.UnitPlan plan = plans.get(currentPlan);
                Draw.draw(Layer.blockOver, () -> ArikothDrawf.construct(this, plan.unit.fullIcon, color, rotdeg() - 90f, progress / plan.time, speedScl, time));
            }
