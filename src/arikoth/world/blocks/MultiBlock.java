...
import arikoth.content.ArikothInnerBlocks;
...
public interface MultiBlock {
    ...
    default boolean checkLink(Tile tile, Team team, int size, int rotation) {
        for (int i = 0; i < linkBlockPos().size; i++) {
            ...
            if (!Build.validPlace(ArikothInnerBlocks.linkEntity[s - 1], team, tile.x + rotated.x, tile.y + rotated.y, 0, false)) {
                return false;
            }
        }
        return true;
    ...
    default void createPlaceholder(Tile tile, int size) {
        ...
        for (int i = 0; i < linkBlockPos().size; i++) {
            ...
            t.setBlock(ArikothInnerBlocks.placeholderEntity[s - 1], tile.team(), 0);
    ...
    default Seq<Building> setLinkBuild(Building building, Block block, Tile tile, Team team, int size, int rotation) {
        ...
        for (int i = 0; i < linkBlockPos().size; i++) {
            ...
            if (!block.outputsLiquid) {
                t.setBlock(ArikothInnerBlocks.linkEntity[s - 1], team, 0);
            } else {
                t.setBlock(ArikothInnerBlocks.linkEntityLiquid[s - 1], team, 0);
            }
