...
public class UnitReforger extends UnitBlock{
    public float constructTime = 60 * 2;
    public Seq<UnitType[]> upgrades = new Seq<>();
    public int[] capacities = {};
    public Color color;

    public UnitReforger(String name){
        super(name);
        ...
        configurable = true;
        config(UnitCommand.class, (UnitReforgerBuild build, UnitCommand command) -> build.command = command);
        configClear((UnitReforgerBuild build) -> build.command = null);
    }
    ...
    public void setBars(){
        ...
        addBar("progress", (UnitReforgerBuild entity) -> new Bar("bar.progress", Pal.ammo, entity::fraction));
        addBar("units", (UnitReforgerBuild e) ->
                new Bar(
    ...
    public class UnitReforgerBuild extends UnitBuild{
        ...
        public void draw(){
            ...
            if(constructing() && hasArrived()){
                Draw.draw(Layer.blockOver, () -> {
                    ...
                    Draw.reset();
                    ArikothDrawf.construct(this, upgrade(payload.unit.type).fullIcon, color, payload.rotation() - 90f, progress / constructTime, speedScl, time);
                });
            }else{
