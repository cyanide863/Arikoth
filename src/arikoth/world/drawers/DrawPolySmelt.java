...
public class DrawPolySmelt extends DrawBlock {
    ...
    public void draw(Building build){
        if(build.warmup() > 0f && flameColor.a > 0.001f){
            ...
            if(drawCenter) Fill.poly(build.x + x, build.y + y, poly, (flameRad * si));

            Draw.color(flameColor, a);
            if(drawCenter) Lines.poly(build.x + x, build.y + y, poly, (flameRad + circleSpace + si) * build.warmup());
